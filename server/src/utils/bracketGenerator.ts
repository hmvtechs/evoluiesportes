...
export function generateSingleElimination(teams: Team[]): BracketMatch[] {
    ...
    while (teamsCopy.length < nextPowerOf2) {
        teamsCopy.push({ id: null as any, name: 'BYE' });
    }
    ...
    while (currentRoundTeams.length > 1) {
        ...
        currentRoundTeams = roundMatches.map(() => ({ id: null as any, name: 'TBD' }));
        round++;
    }
...
export function generateGroupsKnockout(
...
): { groupMatches: Map<number, BracketMatch[]>, knockoutMatches: BracketMatch[] } {
    ...
    const groupMatches = new Map<number, BracketMatch[]>();
