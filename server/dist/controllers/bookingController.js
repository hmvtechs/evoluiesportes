...
exports.getVenueAvailability = exports.listMyBookings = exports.createBooking = void 0;
...
const createBooking = async (req, res) => {
    try {
        ...
        const { data: venue, error: venueError } = await supabase_1.supabase
            .from('Venue')
            .select('*')
            .eq('id', venueId)
            .single();
        ...
        const { count: activeBookings, error: countError } = await supabase_1.supabase
            .from('Booking')
        ...
        const { data: overlap, error: overlapError } = await supabase_1.supabase
            .from('Booking')
            .select('id')
            .eq('venue_id', venueId)
            .neq('status', 'CANCELED')
        ...
        // 7. Create Booking
        const { data: booking, error: createError } = await supabase_1.supabase
            .from('Booking')
            .insert([{
                venue_id: venueId,
                user_id: userId,
...
exports.getVenueAvailability = getVenueAvailability;
