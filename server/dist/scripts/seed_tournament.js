...
async function seedTournament(competitionId) {
    if (!competitionId) {
    ...
    try {
        ...
        const { data: managerAuth, error: managerAuthError } = await supabase_1.supabaseAdmin.auth.admin.createUser({
            email: managerEmail,
        ...
        const { error: orgError } = await supabase_1.supabaseAdmin.rpc('exec_sql', {
        ...
        const { data: orgs, error: orgSelectError } = await supabase_1.supabaseAdmin
            .from('Organization')
            .select('id')
            .eq('cnpj', cnpj)
            .limit(1);
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        const { data: champs } = await supabase_1.supabaseAdmin.from('Championship').select('id').eq('name', champName).limit(1);
        ...
        // Get created teams
        const { data: createdTeams } = await supabase_1.supabaseAdmin
            .from('Team')
            .select('id, category')
        ...
        const { data: createdRegistrations } = await supabase_1.supabaseAdmin
